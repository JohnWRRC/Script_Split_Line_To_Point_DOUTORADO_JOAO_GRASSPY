        ...
        for o in range(len(dirsRaiz2)):
                ...
                for p in dirsRaiz3:
                        ...
                        if p.endswith('.txt'):
                                ...
                                existdir=os.path.exists('Split_'+dirsRaiz3[cont])
                                if existdir==True:
                                        continue
                                else:   
                                        ...
                                        for q in lista_arquivos:
                                                ...
                                                grass.run_command('v.to.points',input=outvect,out=outvect+'_Pt',overwrite=True)
                                                for g in list_rast_resist:
                                                        if g in outvect+'_SpltPnt':
                                                                grass.run_command('g.region',rast=g)
                                                                grass.run_command('v.to.rast',input=outvect+'_Pt',out=outvect+'_PtRt',use="cat",overwrite=True)
                                                                expressao=outvect+'_PtRtVl='+outvect+'_PtRt*'+g
                                                                ...
                                                                print "--------------------------------------------------------------"
                                                                print "--------------------------------------------------------------"
                                                                grass.run_command('r.to.vect',input=outvect+'_PtRtVl',out=outvect+'_PtVl',feature='point',overwrite=True)
                                                                os.chdir(dir1+"\\"+dirsRaiz1[i]+'\\'+dirsRaiz2[o]+'\\'+temp+'\\Split_'+dirsRaiz3[cont])
                                                                
                                                                outvect2=outvect.replace('_shp','_')
                                                                      
                                                                grass.run_command('v.out.ogr',input=outvect+'_PtVl',dsn=outvect2+'_PtVl.shp',type='point')
                                                                grass.run_command('g.remove',flags='f',vect=outvect+'_PtVl,'+outvect+','+outvect+'_Pt')
